...
aide CLI
  ...
  aide start          start the server in the background
  aide stop           stop it
  aide restart        restart it
  aide status         running/stopped + url + reachability
  aide logs [N]       print the last N log lines (default 60)
  aide logs -f        follow the log live (ctrl-c to stop)
  aide logs --clear   truncate the log file
  aide update         git pull, then restart
  aide open           open the browser

windows: aide.cmd   unix/git-bash: ./aide   or just: python app.py
"""
...
def cmd_start(args=()):
    if not _deps_ok():
        ...
        print("(if you installed packages into a different python, run aide with that one)")
        return
    pid = _pid()
    if _running(pid):
        print(f"aide already running  pid={pid}  {_url()}")
        return
    if _port_open():
    ...
    while time.time() < deadline:
        if _port_open():
            print(f"\raide started  pid={proc.pid}  {_url()}        ")
            return
        if proc.poll() is not None:        # process died during startup
            print("\raide failed to start. last log lines:        \n")
            print("\n".join("  " + l for l in _tail(15)) or "  (log empty)")
            PID_FILE.unlink(missing_ok=True)
            return
        print(".", end="", flush=True)
        time.sleep(0.5)
    print(f"\raide is still warming up (pid={proc.pid}).        ")
    print(f"  port {_port()} isn't answering yet — check:  aide logs -f")
...
def cmd_status(args=()):
    ...
    if _running(pid):
        ...
        print(f"aide running   pid={pid}   {_url()}   ({state})")
    elif up:
        print(f"aide running   (untracked pid)   {_url()}   reachable")
    else:
        print("aide stopped")
        PID_FILE.unlink(missing_ok=True)
...
def cmd_update(args=()):
    if not (ROOT / ".git").exists():
        print("not a git checkout — update manually, then  aide restart")
        return
    ...
    try:
        ...
        return
    if r.returncode != 0:
        print("git pull failed (local changes or diverged) — resolve, then  aide restart")
        return
    cmd_restart()
...
def cmd_open(args=()):
    if not _port_open():
        print("aide isn't reachable — start it first with  aide start")
        return
...
def main():
    ...
    if not args or args[0] not in COMMANDS:
        if args:
            print(f"aide: unknown command '{args[0]}'\n", file=sys.stderr)
